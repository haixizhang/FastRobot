...
#include <PDM.h> // Include PDM library included with the Arduino_Apollo3 core
...
// Define LED pin
#define LED_PIN 13 // Change to your LED pin number if different

// Define "C" note frequency and tolerance
#define C_NOTE_FREQ 261.63 // Frequency of middle C in Hz
#define TOLERANCE 10        // Tolerance in Hz

void setup()
{
  Serial.begin(9600);
  Serial.println("SparkFun PDM Example with C Note Detection");
...
void loop()
{
  ...
  // Optional: Remove deep sleep to allow continuous monitoring
  // am_hal_sysctrl_sleep(AM_HAL_SYSCTRL_SLEEP_DEEP);
}

//*****************************************************************************
//
// Analyze and print frequency data, and control LED based on "C" note detection.
//
//*****************************************************************************
void printLoudest(void)
{
  ...
  Serial.printf("Loudest frequency: %d Hz\n", ui32LoudestFrequency);

  //
  // Check if the loudest frequency is within the "C" note range
  //
  if (abs((int)(ui32LoudestFrequency - C_NOTE_FREQ)) <= TOLERANCE)
  {
    digitalWrite(LED_BUILTIN, HIGH); // Turn on LED
    delay(1000); 
    Serial.println("C Note Detected! LED ON");
  }
  else
  {
    digitalWrite(LED_BUILTIN, LOW); // Turn off LED
    delay(1000); 
    Serial.println("C Note Not Detected. LED OFF");
  }
}
